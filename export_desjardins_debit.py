...
RELEVES_DIR = Path(r"C:\Users\sport\Downloads\desjardins_statements")
...
def clean_account_name(raw_name: str) -> str:
    # Enlève la mention (SUITE) et les espaces superflus
...
def main():
    if not RELEVES_DIR.exists():
        print(f"[!] Dossier introuvable : {RELEVES_DIR}")
        return

    pdf_files = sorted(RELEVES_DIR.glob("*.pdf"))
    print(f"Traitement et nettoyage des sous-comptes de {len(pdf_files)} fichiers Desjardins...")
